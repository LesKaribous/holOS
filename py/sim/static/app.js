...
function resizeCanvas() {
  // Read the actual CSS-computed size of the canvas element (flex:1 gives it a size)
  const W = canvas.clientWidth;
  const H = canvas.clientHeight;

  if (W < 10 || H < 10) {
    // Layout not computed yet — retry next frame
    requestAnimationFrame(resizeCanvas);
    return;
  }

  // Only update the draw buffer if dimensions actually changed
...
// ResizeObserver so the canvas responds to sidebar changes etc.
if (window.ResizeObserver) {
  new ResizeObserver(() => { if (activeView === 'map') resizeCanvas(); }).observe(canvas);
}
...
socket.on('state', state => {
  ...
  if (state.hw_connecting) {
    ...
    cgSetRobot(state.hw_mode, state.hw_type);
    _serialSetConnectBusy(false);
    // Update server sub-label: show "Local" when connected (not "SIM")
    const serverSub = document.getElementById('cg-sub-server');
    if (serverSub) serverSub.textContent = state.hw_mode ? 'Local' : 'SIM';
  }
...
function cgSetRobot(hwConnected, hwType) {
  // hwType: 'sim' | 'usb' | 'xbee'  (defaults based on hwConnected)
  const type   = hwType || (hwConnected ? 'usb' : 'sim');
  ...
  if (hwConnected) {
    const label = type === 'xbee' ? 'XBee Radio' : 'USB Serial';
    const modeLabel = type === 'xbee' ? 'Hardware (XBee)' : 'Hardware (USB Serial)';
